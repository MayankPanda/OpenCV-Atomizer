...
def update_image_continuous(label):
    ...
    x_coordinate = int(latest_value * image_width / 100)
    image_with_line = resized_image.copy()
    cv2.line(image_with_line, (x_coordinate, 0), (x_coordinate, image_height), (0, 255, 0), 1)
...
def main_ui(image_name):
    ...
    window = tk.Tk()
    window.title("Calibrate Centreline")
    window.state("zoomed")
    image_label = tk.Label(window)
    image_label.pack()
    slider = tk.Scale(window, from_=0, to=100, orient=tk.HORIZONTAL, resolution=0.001,
                  length=300, width=20, command=lambda value: update_image(value, image_label))
...
def initiate_centerline_calibration(image_name):
    main_ui(image_name)
    global latest_value
    return latest_value
